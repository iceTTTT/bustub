...
ParallelBufferPoolManager::ParallelBufferPoolManager(size_t num_instances, size_t pool_size, DiskManager *disk_manager,
                                                     ...
                                                     num_instances_(num_instances),start_index(0),poolsize_(pool_size)
                                                      {
...
auto ParallelBufferPoolManager::GetBufferPoolManager(page_id_t page_id) -> BufferPoolManager * {
  ...
  return multibufferpool[targetindex];
}
...
auto ParallelBufferPoolManager::UnpinPgImp(page_id_t page_id, bool is_dirty) -> bool {
  ...
  size_t targetindex=page_id%num_instances_;
  if(multibufferpool[targetindex]->UnpinPgImp(page_id,is_dirty))
    return true;
  return false;
}
...
auto ParallelBufferPoolManager::FlushPgImp(page_id_t page_id) -> bool {
  ...
  size_t targetindex=page_id%num_instances_;
  if(multibufferpool[targetindex]->FlushPgImp(page_id))
    return true;
  return false;
}
...
auto ParallelBufferPoolManager::NewPgImp(page_id_t *page_id) -> Page * {
  ...
  {
    /* code */
    ret=multibufferpool[start_index]->NewPgImp(page_id);
    start_index=(start_index+1)%num_instances_;

  } while (ret==nullptr && start_index!=start);
...
auto ParallelBufferPoolManager::DeletePgImp(page_id_t page_id) -> bool {
  ...
  size_t targetindex=page_id%num_instances_;
  if(multibufferpool[targetindex]->DeletePage(page_id))
    return true;
